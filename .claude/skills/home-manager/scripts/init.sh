#!/usr/bin/env bash
# home-manager スキルのセットアップ。
# flake.lock にピンされた rev に合わせて home-manager のソースを /tmp に用意する。
# これにより「読むオプション集合」が実機（インストール済みの世代）と一致し、
# 誤ったオプション提案を防ぐ。
#
...
prepare_source "home-manager" "home-manager"

echo
echo "ソース準備完了:"
echo "  /tmp/home-manager … modules/ (オプション定義+実装), tests/modules/ (使用例)"
